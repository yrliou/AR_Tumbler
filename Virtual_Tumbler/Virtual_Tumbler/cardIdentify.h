...
void findKeypointsDescriptors(cv::Mat grayImage, cv::ORB *orb_detector_, cv::vector<cv::vector<cv::Point>> card_corners, cv::vector<cv::vector<cv::KeyPoint>>& keypoints, cv::vector<cv::Mat>& descriptors);
